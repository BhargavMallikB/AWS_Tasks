...
export async function getUsers(): Promise<User[] | undefined> { // returns Promise of User array
    ...
    try{ // Handling Errors using try & catch blocks
        ...
        return response.data; // prints the data that gets from the response in a tabular format
    } catch (err) { 
        console.error(`Error occured while Fetching Users ${err}`); // any error occurs prints in the console
    }
}

console.table(getUsers()); // Invokes above function to retreive the data
